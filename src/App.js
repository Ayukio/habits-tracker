...
import { List, AddList, Tasks, Badge } from './components';
...
function App() {
  ...
  const onEditTask = (listId, taskObj) => {
    const newTaskText = window.prompt('Your goal title', taskObj.text);
    ...
    axios
      ...
      .catch(() => {
        alert('Can not update habit');
      });
  };

  const onRemoveTask = (listId, taskId) => {
    if (window.confirm('Are you sure to remove this habit?')) {
      const newList = lists.map(item => {
        if (item.id === listId) {
